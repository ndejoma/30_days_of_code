...
func solve(meal_cost float64, tip_percent int32, tax_percent int32) {
	...
	tip_amount := meal_cost * (float64(tip_percent) / 100)
	tax_amount := meal_cost * (float64(tax_percent) / 100)
	total_meal_cost := meal_cost + tip_amount + tax_amount
	fmt.Println(math.Round(total_meal_cost))
}
